 ...
 */
 ...
 /**
  * Modified by CA
  * - Only use the CKEditor context menu on tables, so we leave easy access to system menu, including spell check, on right click
  */
				...
				addTarget: function( element, nativeContextMenuOnCtrl ) {
					element.on( 'contextmenu', function( event ) {
						var domEvent = event.data,
							isCtrlKeyDown = ( CKEDITOR.env.mac ? domEvent.$.metaKey : domEvent.$.ctrlKey );

						if ( nativeContextMenuOnCtrl && isCtrlKeyDown )
							return;

						var cell = new CKEDITOR.dom.elementPath( domEvent.getTarget(), this.editor.editable() ).contains( { 'td': 1, 'th': 1 }, 1 );
						if (!cell) {
							return;
						}
