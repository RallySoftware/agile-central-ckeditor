(function() {
  ...
  function startMentioningKeyEvent(editorInstance, event) {
    if (!editorInstance.isMentioning && event.data.keyCode === mentioningSymbol) {
      ...
      // interesting that we cancel the key event but don't fire change
    }
  }
  ...
  function selectMention(event, uuid, name) { // x
    event.cancel();
    if (uuid) {
      var editorInstance = event.editor;
  ...
  function keyboardInteraction(editorInstance, event) {
    if (editorInstance.isMentioning) {
      ...
      if (editorInstance.handleTimeout) {
            clearTimeout(editorInstance.handleTimeout); // remove; didn't resolve existing problem(s)
      }
      editorInstance.handleTimeout = setTimeout(function() { // remove; didn't resolve existing problem(s)
         editorInstance.acceptingSuggestions = true; // remove; didn't resolve existing problem(s)
      }, 500); // remove; didn't resolve existing problem(s)
      if (keyCode === enterKey) {
        var selected = editorInstance.suggestionList.$.querySelector('.selected');
